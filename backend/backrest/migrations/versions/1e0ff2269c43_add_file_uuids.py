...
def upgrade():
    op.add_column('file', sa.Column('uuid', postgresql.UUID(as_uuid=True), nullable=False))
...
def downgrade():
    op.add_column('file', sa.Column('path', sa.VARCHAR(length=128), autoincrement=False, nullable=False))
