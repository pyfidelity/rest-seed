...
To get going, start by renaming the package.  To do so, simply search and replace all occurences of "foobar", in particular the one in `backend/setup.py`.  The module name ("restbase") should not be changed, though, to make the above-mentioned merging more convenient.
